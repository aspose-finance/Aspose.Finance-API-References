---
title: InlineXbrlDocument.GetArcroleTypeByURI
second_title: .NET API 참조용 Aspose.Finance
description: InlineXbrlDocument 방법. 지정된 uri가 있는 ArcroleType을 가져옵니다.
type: docs
weight: 170
url: /ko/net/aspose.finance.xbrl.inline/inlinexbrldocument/getarcroletypebyuri/
---
## InlineXbrlDocument.GetArcroleTypeByURI method

지정된 uri가 있는 ArcroleType을 가져옵니다.

```csharp
public ArcroleType GetArcroleTypeByURI(string arcroleTypeUri)
```

| 모수 | 유형 | 설명 |
| --- | --- | --- |
| arcroleTypeUri | String | 아크 역할 유형 uri. |

### 반환 값

아크롤타입.

### 또한보십시오

* class [ArcroleType](../../../aspose.finance.xbrl/arcroletype/)
* class [InlineXbrlDocument](../)
* 네임스페이스 [Aspose.Finance.Xbrl.Inline](../../inlinexbrldocument/)
* 집회 [Aspose.Finance](../../../)


