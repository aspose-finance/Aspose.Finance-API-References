---
title: "PresentmentDirMessageSetV1"
second_title: "مرجع واجهة برمجة تطبيقات Aspose.Finance لـ .NET"
description: "الإصدار 1 من مجموعة رسائل دليل الفاتور."
type: docs
weight: 4650
url: /ar/net/aspose.finance.ofx.profile/presentmentdirmessagesetv1/
---
## PresentmentDirMessageSetV1 class

الإصدار 1 من مجموعة رسائل دليل الفاتور.

```csharp
public class PresentmentDirMessageSetV1 : AbstractMessageSetVersion
```

## المنشئات

| الاسم | الوصف |
| --- | --- |
| [PresentmentDirMessageSetV1](presentmentdirmessagesetv1)() | يُنشئ مثيلًا جديدًا من الفئة [`PresentmentDirMessageSetV1`](../presentmentdirmessagesetv1). |

## الخصائص

| الاسم | الوصف |
| --- | --- |
| [MessageSetCore](../../aspose.finance.ofx.profile/abstractmessagesetversion/messagesetcore) { get; set; } | الحصول أو تعيين [`MessageSetCore`](../abstractmessagesetversion/messagesetcore). |
| [PresentmentDirProfile](../../aspose.finance.ofx.profile/presentmentdirmessagesetv1/presentmentdirprofile) { get; set; } | الحصول أو تعيين الـ [`PresentmentDirProfile`](./presentmentdirprofile). |

### انظر أيضًا

* class [AbstractMessageSetVersion](../abstractmessagesetversion)
* namespace [Aspose.Finance.Ofx.Profile](../../aspose.finance.ofx.profile)
* assembly [Aspose.Finance](../../)

<!-- لا تقم بالتعديل: تم إنشاؤه بواسطة xmldocmd لـ Aspose.Finance.dll -->
