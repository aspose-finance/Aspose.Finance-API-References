---
title: "PresentmentDlvMessageSetV1"
second_title: "مرجع واجهة برمجة تطبيقات Aspose.Finance لـ .NET"
description: "الإصدار 1 من مجموعة رسائل تسليم الفاتور."
type: docs
weight: 4680
url: /ar/net/aspose.finance.ofx.profile/presentmentdlvmessagesetv1/
---
## PresentmentDlvMessageSetV1 class

الإصدار 1 من مجموعة رسائل تسليم الفاتور.

```csharp
public class PresentmentDlvMessageSetV1 : AbstractMessageSetVersion
```

## المنشئات

| الاسم | الوصف |
| --- | --- |
| [PresentmentDlvMessageSetV1](presentmentdlvmessagesetv1)() | ينشئ مثلاً جديداً من الفئة [`PresentmentDlvMessageSetV1`](../presentmentdlvmessagesetv1). |

## الخصائص

| الاسم | الوصف |
| --- | --- |
| [EmailProfile](../../aspose.finance.ofx.profile/presentmentdlvmessagesetv1/emailprofile) { get; set; } | الحصول أو تعيين الـ[`EmailProfile`](./emailprofile). |
| [MessageSetCore](../../aspose.finance.ofx.profile/abstractmessagesetversion/messagesetcore) { get; set; } | الحصول أو تعيين [`MessageSetCore`](../abstractmessagesetversion/messagesetcore). |
| [PresentmentDlvProfile](../../aspose.finance.ofx.profile/presentmentdlvmessagesetv1/presentmentdlvprofile) { get; set; } | يحصل أو يعيّن الـ [`PresentmentDlvProfile`](./presentmentdlvprofile). |

### انظر أيضًا

* class [AbstractMessageSetVersion](../abstractmessagesetversion)
* namespace [Aspose.Finance.Ofx.Profile](../../aspose.finance.ofx.profile)
* assembly [Aspose.Finance](../../)

<!-- لا تقم بالتعديل: تم إنشاؤه بواسطة xmldocmd لـ Aspose.Finance.dll -->
