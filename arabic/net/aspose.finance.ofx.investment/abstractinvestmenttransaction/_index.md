---
title: "AbstractInvestmentTransaction"
second_title: "مرجع واجهة برمجة تطبيقات Aspose.Finance لـ .NET"
description: "الفئة الأساسية المجردة للمعاملات الاستثمارية غير المتعلقة بالشراء/البيع."
type: docs
weight: 2640
url: /ar/net/aspose.finance.ofx.investment/abstractinvestmenttransaction/
---
## AbstractInvestmentTransaction class

فئة أساسية مجردة للفئات المتعلقة بمعاملات الاستثمار (ليس شراء/بيع).

```csharp
public class AbstractInvestmentTransaction : AbstractInvestmentTransactionBase
```

## الخصائص

| الاسم | الوصف |
| --- | --- |
| [InvestmentTransaction](../../aspose.finance.ofx.investment/abstractinvestmenttransaction/investmenttransaction) { get; set; } | الحصول أو تعيين [`InvestmentTransaction`](./investmenttransaction). |

### انظر أيضًا

* class [AbstractInvestmentTransactionBase](../abstractinvestmenttransactionbase)
* namespace [Aspose.Finance.Ofx.Investment](../../aspose.finance.ofx.investment)
* assembly [Aspose.Finance](../../)

<!-- لا تقم بالتعديل: تم إنشاؤه بواسطة xmldocmd لـ Aspose.Finance.dll -->
