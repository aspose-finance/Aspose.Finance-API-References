...
description: Aspose.Finance.Xbrl.Dom.Html.HTMLObjectElement クラス. クラスは一般的な埋め込みオブジェクトを表します原則としてオブジェクト要素のすべてのプロパティは読み書き可能ですが環境によっては基になるオブジェクトがインスタンス化されると一部のプロパティが読み取り専用になる場合があります HTML 4.01 の OBJECT 要素の定義を参照してください
type: docs
weight: 7100
url: /ja/net/aspose.finance.xbrl.dom.html/htmlobjectelement/
---
## HTMLObjectElement class

クラスは一般的な埋め込みオブジェクトを表します。原則として、オブジェクト要素のすべてのプロパティは読み書き可能ですが、環境によっては、基になるオブジェクトがインスタンス化されると一部のプロパティが読み取り専用になる場合があります。 HTML 4.01 の OBJECT 要素の定義を参照してください。
