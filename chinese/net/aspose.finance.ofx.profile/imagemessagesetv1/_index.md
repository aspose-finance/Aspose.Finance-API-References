---
title: "ImageMessageSetV1"
second_title: "Aspose.Finance 适用于 .NET API 参考"
description: "图像消息集版本1。"
type: docs
weight: 4530
url: /zh/net/aspose.finance.ofx.profile/imagemessagesetv1/
---
## ImageMessageSetV1 class

图像消息集版本1。

```csharp
public class ImageMessageSetV1 : AbstractMessageSetVersion
```

## 构造函数

| 名称 | 描述 |
| --- | --- |
| [ImageMessageSetV1](imagemessagesetv1)() | 初始化 [`ImageMessageSetV1`](../imagemessagesetv1) 类的新实例。 |

## 属性

| 名称 | 描述 |
| --- | --- |
| [DefaultImageDelay](../../aspose.finance.ofx.profile/imagemessagesetv1/defaultimagedelay) { get; set; } | 获取或设置从 ServerDate（针对对账单图像）或 PostedDate（针对交易图像）起的默认日历天数，图像将在此后可用。 |
| [DefaultImageTtl](../../aspose.finance.ofx.profile/imagemessagesetv1/defaultimagettl) { get; set; } | 获取或设置图像一旦可用后在主机上保持可用的默认日历天数。 |
| [MessageSetCore](../../aspose.finance.ofx.profile/abstractmessagesetversion/messagesetcore) { get; set; } | 获取或设置 [`MessageSetCore`](../abstractmessagesetversion/messagesetcore)。 |

### 另请参阅

* class [AbstractMessageSetVersion](../abstractmessagesetversion)
* namespace [Aspose.Finance.Ofx.Profile](../../aspose.finance.ofx.profile)
* assembly [Aspose.Finance](../../)

<!-- 请勿编辑：由 xmldocmd 为 Aspose.Finance.dll 生成 -->
