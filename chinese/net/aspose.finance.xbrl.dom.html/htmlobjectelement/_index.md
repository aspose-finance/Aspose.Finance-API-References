...
description: 该类表示通用嵌入对象原则上对象元素上的所有属性都是可读写的但在某些环境中一旦底层对象实例化某些属性可能是只读的请参阅 HTML 4.01 中的 OBJECT 元素定义
type: docs
weight: 7070
...
该类表示通用嵌入对象。原则上，对象元素上的所有属性都是可读写的，但在某些环境中，一旦底层对象实例化，某些属性可能是只读的。请参阅 HTML 4.01 中的 OBJECT 元素定义。
