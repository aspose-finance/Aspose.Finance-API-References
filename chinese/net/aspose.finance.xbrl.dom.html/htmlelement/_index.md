...
description: 所有 HTML 元素类都派生自此类仅暴露 HTML 核心属性的元素由该类表示这些元素如下 specialSUBSUPSPANBDOfontTTIBUSSTRIKEBIGSMALL 短语EMSTRONG  DFN CODE SAMP KBD VAR CITE ACRONYM ABBR 列表DD DTNOFRAMES NOSCRIPTADDRESS CENTERT.
type: docs
weight: 6830
...
所有 HTML 元素类都派生自此类。仅暴露 HTML 核心属性的元素由该类表示，这些元素如下： special：SUB、SUP、SPAN、BDOfont：TT、I、B、U、S、STRIKE、BIG、SMALL 短语：EM、STRONG , DFN, CODE, SAMP, KBD, VAR, CITE, ACRONYM, ABBR 列表：DD, DTNOFRAMES, NOSCRIPTADDRESS, CENTERT.
