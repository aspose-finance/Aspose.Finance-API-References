...
description: 类代表表单控件根据查看页面的环境文件上传输入类型的 value 属性可能是只读的对于密码输入类型可能会屏蔽返回的实际值以防止未经授权的使用请参阅 HTML 4.01 中的 INPUT 元素定义
type: docs
weight: 6950
...
类代表表单控件。根据查看页面的环境，文件上传输入类型的 value 属性可能是只读的。对于“密码”输入类型，可能会屏蔽返回的实际值，以防止未经授权的使用。请参阅 HTML 4.01 中的 INPUT 元素定义。
